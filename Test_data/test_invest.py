...
test_data=DoExcel().data_read('register')
# print(test_data)
@ddt
class TestCase(unittest.TestCase):
    ...
    def test_http_request(self,case):
        ...
        resp = TestRequest().testrequest(Method,Url,param)
        #获取请求结果
        try:
            ...
            DoExcel().data_write('register',Caseid+1,9,resp.text)
            TestLog().info("预期结果:：{}".format(Expect_Result))
            TestLog().info("回写测试结论：{}".format(TestResult))
            DoExcel().data_write('register',Caseid+1,10,TestResult)
